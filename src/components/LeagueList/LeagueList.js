...
class LeagueList extends Component {
    ...
    render() {
        ...
        const sortElements = <div className={classes.SortByLine}>
        ...
        const content = this.props.games.slice(0);
        ...
        if (this.props.dateSet !== "") {
            const sp = this.props.dateSet.split("-");
        ...
        const c = (content.map((text, index) => {
            ...
            if (currentDay.getDate() === todayDay) {
                hasGames = true;
                hours = currentDay.getHours() < 10 ? '0' + currentDay.getHours() : currentDay.getHours();
                minutes = currentDay.getMinutes() < 10 ? '0' + currentDay.getMinutes() : currentDay.getMinutes();
                ...
                if (countryCompetition !== (text.country + " - " + text.competition)) {
                    ...
                    if (brojac === 7 || brojac === 15) {
                        contentCountry = [<CountrySingle key="cs" c={countryCompetition} co={text.country} />,
                        <MatchSingle
                            key={index}
                            t={hours + ":" + minutes}
                            h={text.homeTeam}
        ...
        let hasGamesDIv = "";
        if (!hasGames) {
            hasGamesDIv = <div className={classes.noGames}>There are no available games for this date.</div>;
        }
        ...
        return (
            <div className={classes.Container}>
                <div className={classes.topExpl}>
                    Below you can find detailed betting stats and picks for all today's games.
            </div>
                {sortElements}
                {c}
                {hasGamesDIv}
