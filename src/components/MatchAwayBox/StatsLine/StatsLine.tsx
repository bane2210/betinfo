...
const StatsLine = (props: Prop) => {
  const [state, setState] = useState({
    open: false,
  });

  const styleCalc = (x: number | string) => {
    if (x < 40) return "gray";
  ...
  if (props.gameList.reverse().length > 0 && state.open) {
    gameList = props.gameList.map((element: any, index: any) => {
  ...
  if (state.open) {
    content = (
      <li className={classes.StatsLine} onClick={toglleOpen}>
        <div className={classes.a}>
          <div style={{ color: styleCalc(br) }} className={classes.br}>
            {br + "%"}
          </div>
    ...
    if (props.gameList.length > 0) {
      ...
      if (props.setList) {
        content = (
          <li className={classes.StatsLine}>
