...
interface Prop {
  isTop: boolean;
  borderArray: { name: string; br: number }[];
  date: string;
  sqlContent: any[];
  arrTips: string[];
  arrNames: any[];
...
*/
  ...
  const setStateDefaulf = (start: number, end: number) => {
    setState((prevState) => {
      return {
        ...prevState,
        start: start,
  ...
  let contentTips: "" | JSX.Element | JSX.Element[] = "";
  let contentGamesInner: "" | JSX.Element | JSX.Element[] = "";
  ...
  let arrImutable: any[] = [];
  let arr: any[] = [];
  let contentNext: JSX.Element | "" | JSX.Element[] = "";
  const isTop = props.isTop;
  let sortElements: "" | JSX.Element | JSX.Element[] = "";
  ...
  if (sqlContent.length > 0) {
    if (!isTop) {
      ...
      if (state.sortBy === 1) {
        ...
        arr = [...arrImutable].sort((a: any, b: any) => {
  ...
  const recomendedArr: any[] = [];
  const couldTryArr: any[] = [];
        ...
        <div className={classes.AllGamesLine}>
          <div className={classes.overallButtons}>
            ...
            <OverallButton
              o={state.couldTry}
              click={setCouldTry}
              name={!isTop ? "You Could Try" : "Chance 80% - 90%"}
            />
          </div>
          {percCount}
          {dottedLine}
