...
const GenerateTicket: React.FC<{
  ...
  const generateTicketFun = () => {
    ...
    arr.forEach((element) => {
      ...
      if (marketValues.includes(element.name)) {
        ...
        element.content.forEach((game: any) => {
    ...
    let contentGames: string | JSX.Element[] = "0";
  ...
  const setStateOddsTime = (value: string, br: number) => {
    if (br === 1) {
      setState((prevState) => {
        return {
          ...prevState,
  ...
  let generateButton: "" | JSX.Element = "";
  ...
  let contentTicket: string | JSX.Element | JSX.Element[] = "";
